...
printf '{"env":{"EXISTING":"1","PATH":"/tmp/old-safe-install/bin:${PATH}"}}\n' > "$tmp_dir/claude/settings.json"
printf '# safe-install dependency isolation\nsource /tmp/old-safe-install/activate.sh\n' > "$tmp_dir/zshrc"
...
grep -q -- 'UV_CACHE_DIR=/safe-install/uv-cache' <<< "$uv_dry"
grep -q -- 'UV_PYTHON_INSTALL_DIR=/safe-install/uv-python' <<< "$uv_dry"
grep -q -- 'dst=/safe-install' <<< "$uv_dry"
...
grep -q 'safe-install: added activation' <<< "$init_again_out"
grep -q 'restart Claude Code' <<< "$init_out"
grep -q "source $repo_root/activate.sh" "$tmp_dir/zshrc"
[[ "$(grep -c 'safe-install/activate.sh' "$tmp_dir/zshrc")" == "1" ]]
